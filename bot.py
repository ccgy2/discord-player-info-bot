...
async def is_mc_username(nick str) - bool
    ...
    url = fhttpsapi.mojang.comusersprofilesminecraft{quote_plus(nick)}
    try
...
def mc_avatar_url(nick str, size int = 128) - str
    if not nick
        return 
    return fhttpsminotar.netavatar{quote_plus(nick)}{size}.png

def mc_body_url(nick str, width int = 400) - str
    if not nick
        return 
    return fhttpsminotar.netbody{quote_plus(nick)}{width}.png
...
def pitch_has_power(pitch str) - bool
    return bool(re.search(r'(sd+s)$', pitch))
...
def format_registrar_field_and_avatar(created_by dict) - (str, Optional[str])
    ...
    if discr
        ...
        name_repr = f{display}
    return f{name_repr}nID {uid}, avatar_url

def make_player_embed(data dict, context Optional[dict] = None) - discord.Embed
    
    data player document dict
    context optional dict, e.g., {actioncreateappendedit} to choose color
    ...
    if pitch_types
        pitches_display = n.join([f- {p} for p in (pitch_types[20])])
    else
        pitches_display = -
    ...
    if reg_text
        ...
        embed.set_author(name=f등록자 {reg_text.splitlines()[0]}, icon_url=reg_avatar)
    ...
    embed.add_field(name=f구종 ({len(pitch_types)}), value=pitches_display, inline=False)

    # meta
    created = data.get('created_at', '-')
    updated = data.get('updated_at', '-')
    embed.set_footer(text=f등록 {short_time(created)}  수정 {short_time(updated)})
...
등록추가대량등록
`{BOT}등록` - 여러 블록(개행)으로 붙여넣어 등록. (예 닉네임 (폼) n 구종...)
`{BOT}추가 nick이름팀포지션구종1,구종2폼` - 한 명 추가 (파이프 형식)
`{BOT}추가 nickn구종 구종` - 닉네임 + 다음 라인 구종 형식도 가능. (이미 존재하면 구종을 append)
...
구종 구종, 구종
- 블록형으로 보내면 해당 선수의 폼구종포지션팀을 교체(단, 팀폼 미기재 시 기존값 유지).
...
`{BOT}기록보기 닉네임`  
`{BOT}기록리셋 닉네임 type` - type battingpitchingall
...
def split_into_blocks(text str) - List[List[str]]
    raw_blocks = re.split(r'nsn', text.strip(), flags=re.MULTILINE)
    blocks = []
    for b in raw_blocks
...
def parse_pitch_line(pitch_line str) - List[str]
    ...
    tokens = re.findall(r'([^s,]+((sd+s))[^s,]+)', pitch_line.strip())
    out = []
    for tok in tokens
...
async def add_one_cmd(ctx, , payload str)
        ...
        try
            ...
            if len(block_lines) == 1 and '' in block_lines[0]
                ...
                if VERIFY_MC and not exists
                    ...
                    if not valid
                        failed.append(f블록 {i} `{raw_nick}` 은(는) 마인크래프트 계정 아님)
                        continue
            ...
            if exists
                ...
                data = {
                    ...
                    position parsed.get(position, NA),
    ...
    if failed
        summary.add_field(name=오류 예시 (최대 10), value=n.join(failed[10]), inline=False)
...
async def bulk_register_cmd(ctx, , bulk_text str = None)
    ...
    for i, block in enumerate(blocks, start=1)
        try
            ...
            if VERIFY_MC and not exists
                ...
                if not valid
                    errors.append(f블록 {i} `{raw_nick}` 은(는) 마인크래프트 계정 아님)
                    continue
    ...
    if errors
        summary_embed.add_field(name=오류 예시 (최대 10), value=n.join(errors[10]), inline=False)
...
async def edit_cmd(ctx, , payload str)
    ...
    # 블록형 판단 payload에 newline이 있거나 괄호대괄호 포함되어 블록으로 생각
    if n in payload or ( in payload or [ in payload
        ...
        # prepare update replace fields (formteampitch_typespositionname)
        updates = {}
        ...
        updates[position] = parsed.get(position, old.get(position, NA))
...
# ---------- 나머지 명령들 (이적영입삭제구종삭제팀팀삭제목록트레이드웨이버방출기록) ----------
@bot.command(name=이적)
...
async def team_cmd(ctx, , teamname str)
    ...
    roster = t.get(roster, [])
    if roster
        await ctx.send(f{team_norm} — 로스터 ({len(roster)})n + , .join(roster[200]))
    else
...
async def delete_team_cmd(ctx, , teamname str)
    ...
    try
        ...
        t_ref.delete()
        embed = discord.Embed(title=팀 삭제 완료, description=f팀 `{team_norm}` 을(를) 삭제하고 해당 선수들을 FA로 이동했습니다., color=discord.Color.red(), timestamp=datetime.now(timezone.utc))
        ...
        if errors
            embed.add_field(name=오류 예시 (최대 10), value=n.join(errors[10]), inline=False)
        await ctx.send(embed=embed)
    except Exception as e
    ...
    if kind == players
        ...
        if not lines
            ...
            for i in range(0, len(text), chunk_size)
                await ctx.send(text[ii+chunk_size])
    elif kind == teams
        docs = db.collection(teams).order_by(name).stream()
        lines = [d.to_dict().get(name,-) for d in docs]
        await ctx.send(팀 목록n + (, .join(lines) if lines else 없음))
    else
        await ctx.send(사용법 `!목록 playersteams`)
...
async def view_records_cmd(ctx, nick str)
    ...
    if batting
        ...
        avg = (total_H  total_AB) if total_AB0 else 0
        lines.append(f타자 기록 {len(batting)}경기 — PA{total_PA} AB{total_AB} H{total_H} AVG{avg.3f})
    else
        lines.append(타자 기록 없음)
    if pitching
        total_IP = sum(float(x.get(IP,0)) for x in pitching)
        total_ER = sum(int(x.get(ER,0)) for x in pitching)
        era = (total_ER  9  total_IP) if total_IP0 else 0
        lines.append(f투수 기록 {len(pitching)}경기 — IP{total_IP} ER{total_ER} ERA{era.2f})
    else
...
async def reset_records_cmd(ctx, nick str, typ str)
    ...
    try
        if typ == batting
            ...
            await ctx.send(TYPE 오류 battingpitchingall 중 하나를 사용하세요.)
            return
...
async def on_command_error(ctx, error)
    if isinstance(error, commands.MissingRequiredArgument)
        ...
        await ctx.send(f명령 실행 중 오류가 발생했습니다 `{error}`)
